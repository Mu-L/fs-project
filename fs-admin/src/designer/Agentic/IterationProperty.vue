...
// 错误处理选项：错误时终止、忽略错误并继续、移除错误输出
const errorModes = computed<any[]>(() => props.config?.iterationErrorModes ?? config.iterationErrorModes ?? [])
      ...
      <el-form :model="model" label-position="top">
        ...
        <SectionSlice title="迭代配置">
          ...
          <el-form-item label="输出变量">
            <VariableSelect
              v-model="model.data.outputVariable"
              :instance="$props.instance"
              :active-item="model"
              inner
              placeholder="请选择容器内节点输出的变量" />
          </el-form-item>
          <el-form-item label="输出变量名">
            <el-input v-model="model.data.outputName" placeholder="如 output，收集各次输出组成的数组" />
          </el-form-item>
          ...
          <el-form-item label="错误处理">
            <el-select v-model="model.data.errorMode" placeholder="请选择">
              <el-option :key="item.value" :value="item.value" :label="item.label" v-for="item in errorModes" />
            </el-select>
          </el-form-item>
          <el-form-item label="">
            <tip-text>
              ...
              引用当前迭代的元素；每次迭代收集的输出变量会组成
              <em>{{ model.data.outputName || 'output' }}</em>
              的值数组
            </tip-text>
          </el-form-item>
        </SectionSlice>
