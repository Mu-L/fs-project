...
import PageApi from '@/api/spider/PageApi';
...
const columns = ref([
  ...
  { prop: 'code', label: '页面编码' },
  { prop: 'url', label: '链接地址' },
  { prop: 'priority', label: '优先级' },
  { prop: 'forceMode', label: '强制采集' },
  { prop: 'charset', label: '页面编码', hide: true },
  { prop: 'collection', label: '数据存储' },
  { prop: 'bucket', label: '文件存储' },
  { prop: 'connectTimeout', label: '连接超时', hide: true },
  { prop: 'socketTimeout', label: '执行超时', hide: true },
  { prop: 'iterateCount', label: '翻页深度', hide: true },
  { prop: 'retryCount', label: '重试次数', hide: true },
...
const handleRefresh = (filter2query: boolean, keepPage: boolean) => {
  ...
  loading.value = true
  PageApi.list(filters.value).then((result: any) => {
...
onMounted(() => {
  ...
  PageApi.config().then((result: any) => {
...
const handleReload = () => {
  ...
  PageApi.info({ id: form.value.id }).then(result => {
...
const handleSubmit = () => {
  formRef.value?.validate((valid: boolean) => {
    ...
    PageApi.save(form.value, { success: true }).then(result => {
      handleRefresh(false, true)
...
const handleDelete = () => {
  TableUtil.selection(selection.value).then((ids: any) => {
    loading.value = true
    PageApi.delete(ids, { success: true }).then(() => {
      handleRefresh(false, true)
    }).catch(() => {
      loading.value = false
    })
...
<template>
  <el-card :bordered="false" shadow="never" class="fs-table-search" v-show="searchable">
    <form-search ref="filterRef" :model="filters">
      ...
      <form-search-item label="编码" prop="code">
  ...
  <el-card :bordered="false" shadow="never" class="fs-table-card">
    <div class="fs-table-toolbar flex-between">
      <el-space>
        <button-add v-permit="'spider:page:add'" @click="handleAdd" />
        <button-delete v-permit="'spider:page:delete'" :disabled="selection.length === 0" @click="handleDelete" />
      </el-space>
      <el-space>
    ...
    >
      ...
      <el-table-column label="操作">
        <template #default="scope">
          <el-button link @click="handleEdit(scope)" v-permit="'spider:page:modify'">编辑</el-button>
        </template>
      </el-table-column>
    </el-table>
  ...
  <el-drawer v-model="formVisible" :close-on-click-modal="false" :show-close="false" :destroy-on-close="true" size="50%">
    ...
    <el-form ref="formRef" :model="form" :rules="rules" label-width="auto">
      <el-descriptions border :column="2">
        <el-descriptions-item label="页面名称" prop="name">
          <el-input v-model="form.name" />
        </el-descriptions-item>
        <el-descriptions-item label="页面编码" prop="code">
          <el-input v-model="form.code" placeholder="通过编码相互调度引用" />
        </el-descriptions-item>
        <el-descriptions-item label="链接地址" prop="url" :span="2">
          <el-input type="textarea" v-model="form.url" placeholder="请求链接地址，支持{变量}占位符" />
        </el-descriptions-item>
        ...
        <el-descriptions-item label="采集配置" :span="2">
          <el-checkbox v-model="form.forceMode">强制采集</el-checkbox>
        </el-descriptions-item>
        ...
        <el-descriptions-item label="文件存储" prop="bucket">
        ...
        <el-descriptions-item label="翻页深度">
        ...
        <el-descriptions-item label="请求头" :span="2">
          <el-input type="textarea" v-model="form.headers" placeholder="JSON格式的键值对" :rows="5" />
        </el-descriptions-item>
