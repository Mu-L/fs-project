...
import SiteApi from '@/api/spider/SiteApi';
...
onMounted(() => {
  ...
  SiteApi.config().then((result: any) => {
...
const handleReload = () => {
  ...
  SiteApi.info({ id: form.value.id }).then(result => {
...
const handleSubmit = () => {
  formRef.value?.validate((valid: boolean) => {
    ...
    SiteApi.save(form.value, { success: true }).then(result => {
      handleRefresh(false, true)
...
const handleDelete = () => {
  TableUtil.selection(selection.value).then((ids: any) => {
    loading.value = true
    SiteApi.delete(ids, { success: true }).then(() => {
      handleRefresh(false, true)
    }).catch(() => {
      loading.value = false
    })
...
<template>
  ...
  <el-card :bordered="false" shadow="never" class="fs-table-card">
    <div class="fs-table-toolbar flex-between">
      <el-space>
        <button-add v-permit="'spider:site:add'" @click="handleAdd" />
        <button-delete v-permit="'spider:site:delete'" :disabled="selection.length === 0" @click="handleDelete" />
      </el-space>
      <el-space>
    ...
    >
      ...
      <el-table-column label="操作">
        <template #default="scope">
          <el-button link @click="handleEdit(scope)" v-permit="'spider:site:modify'">编辑</el-button>
        </template>
      </el-table-column>
    </el-table>
  ...
  <el-drawer v-model="formVisible" :close-on-click-modal="false" :show-close="false" :destroy-on-close="true">
    <template #header="{ close, titleId, titleClass }">
      <h4 :id="titleId" :class="titleClass">站点配置</h4>
      <el-space>
    ...
    <el-form ref="formRef" :model="form" :rules="rules" label-width="auto">
      ...
      <el-form-item label="站点域名" prop="domain">
        <el-input v-model="form.domain" placeholder="*为默认兜底策略" />
      </el-form-item>
      <el-form-item label="页面编码" prop="charset">
        <el-input v-model="form.charset" placeholder="默认utf-8" />
      </el-form-item>
      <el-form-item label="数据存储" prop="collection">
