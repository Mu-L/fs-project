...
import WhitelistApi from '@/api/spider/WhitelistApi';
...
const handleRefresh = (filter2query: boolean, keepPage: boolean) => {
  ...
  loading.value = true
  WhitelistApi.list(filters.value).then((result: any) => {
...
onMounted(() => {
  ...
  WhitelistApi.config().then((result: any) => {
...
const handleReload = () => {
  ...
  WhitelistApi.info({ id: form.value.id }).then(result => {
...
const handleSubmit = () => {
  formRef.value?.validate((valid: boolean) => {
    ...
    WhitelistApi.save(form.value, { success: true }).then(result => {
      handleRefresh(false, true)
...
const handleDelete = () => {
  TableUtil.selection(selection.value).then((ids: any) => {
    loading.value = true
    WhitelistApi.delete(ids, { success: true }).then(() => {
      handleRefresh(false, true)
    }).catch(() => {
      loading.value = false
    })
...
<template>
  ...
  <el-card :bordered="false" shadow="never" class="fs-table-card">
    <div class="fs-table-toolbar flex-between">
      <el-space>
        <button-add v-permit="'spider:whitelist:add'" @click="handleAdd" />
        <button-delete v-permit="'spider:whitelist:delete'" :disabled="selection.length === 0" @click="handleDelete" />
      </el-space>
      <el-space>
    ...
    >
      ...
      <el-table-column label="操作">
        <template #default="scope">
          <el-button link @click="handleEdit(scope)" v-permit="'spider:whitelist:modify'">编辑</el-button>
        </template>
      </el-table-column>
    </el-table>
  ...
  <el-drawer v-model="formVisible" :close-on-click-modal="false" :show-close="false" :destroy-on-close="true">
    <template #header="{ close, titleId, titleClass }">
      <h4 :id="titleId" :class="titleClass">白名单</h4>
      <el-space>
