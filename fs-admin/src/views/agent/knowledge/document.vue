...
const handleDownload = (record: any) => {
  window.open(baseURL + '/agent/knowledgeDocument/download?id=' + encodeURIComponent(record.id))
}
</script>
...
<template>
  <el-card :bordered="false" shadow="never" class="fs-table-search" v-show="searchable">
    <el-space>
      <LayoutBack to="/agent/knowledge/list" />
  ...
  <el-card :bordered="false" shadow="never" class="fs-table-card">
    ...
    >
      ...
      <TableColumn :columns="columns">
        <template #name="scope">
          <el-link type="primary" underline="never" @click="router.push({ path: '/agent/knowledge/segment', query: { documentId: scope.row.id, knowledgeId: route.query.knowledgeId } })">{{ scope.row.name }}</el-link>
        </template>
        <template #role="scope">
      ...
      <el-table-column label="操作">
        <template #default="scope">
          ...
          <el-button link @click="handleDownload(scope.row)" v-permit="'agent:knowledge:'" v-if="scope.row.fileId">下载</el-button>
        </template>
      </el-table-column>
    </el-table>
