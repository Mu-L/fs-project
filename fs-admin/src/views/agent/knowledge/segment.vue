...
import KnowledgeSegmentApi from '@/api/agent/KnowledgeSegmentApi';
...
<template>
  <el-card :bordered="false" shadow="never" class="fs-table-search" v-show="searchable">
    <el-space v-if="route.query.documentId">
      <LayoutBack :to="'/agent/knowledge/document?knowledgeId=' + route.query.knowledgeId" />
        ...
        <template #default="scope">
          <div class="expand-content">
            <div class="expand-toolbar" v-if="scope.row.chunks?.length">
              ...
              <el-space>
                <button-add v-permit="'agent:knowledge:add'" @click="handleChunkAdd(scope.row.id)" size="small" />
                <button-delete v-permit="'agent:knowledge:delete'" :disabled="!(chunkSelection[scope.row.id] || []).length" @click="handleChunkDelete(scope.row.id)" size="small" />
              </el-space>
            </div>
            <div v-if="scope.row.chunks?.length" class="chunk-cards">
              ...
              >
                <div class="chunk-card__header">
                  ...
                  <el-button link type="primary" @click.stop="handleChunkEdit(scope.row.id, chunk)" v-permit="'agent:knowledge:modify'">编辑</el-button>
                </div>
            ...
            <el-empty v-else description="暂无分块" :image-size="60">
              <button-add v-permit="'agent:knowledge:add'" @click="handleChunkAdd(scope.row.id)" />
            </el-empty>
          </div>
        </template>
      </el-table-column>
