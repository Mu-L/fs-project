...
import SkillVersionApi from '@/api/agent/SkillVersionApi';
...
const handleDownload = (record: any) => {
  window.open(baseURL + '/agent/skillVersion/download?id=' + encodeURIComponent(record.id))
}
...
<template>
  <el-card :bordered="false" shadow="never" class="fs-table-search" v-show="searchable">
    <el-space>
      <LayoutBack to="/agent/plugin/skill" />
  ...
  <el-card :bordered="false" shadow="never" class="fs-table-card">
    <div class="fs-table-toolbar flex-between">
      <el-space>
        <el-button type="success" :icon="ElementPlusIcons.UploadFilled" v-permit="'agent:skill:add'" @click="handleOpenUpload">上传版本</el-button>
    ...
    >
      ...
      <el-table-column label="操作">
        <template #default="scope">
          ...
          <el-button link @click="handleDownload(scope.row)" v-permit="'agent:skill:'" v-if="scope.row.fileId">下载</el-button>
        </template>
      </el-table-column>
    </el-table>
