...
<template>
  ...
  <el-card :bordered="false" shadow="never" class="fs-table-card">
    <div class="fs-table-toolbar flex-between">
      <el-space>
        <button-add v-permit="'agent:agent:add'" @click="handleAdd" />
        <button-delete v-permit="'agent:agent:delete'" :disabled="selection.length === 0" @click="handleDelete" />
      </el-space>
      <el-space>
    ...
    >
      ...
      <el-table-column label="操作">
        <template #default="scope">
          <el-button link @click="handleShow(scope)" v-permit="'agent:agent:'">查看</el-button>
          <el-button link @click="handleEdit(scope)" v-permit="'agent:agent:modify'">编辑</el-button>
        </template>
      </el-table-column>
    </el-table>
