...
const handleRefresh = (filter2query: boolean, keepPage: boolean) => {
  ...
  loading.value = true
  MatrixApi.list(filters.value).then((result: any) => {
...
onMounted(() => {
  ...
  MatrixApi.config().then((result: any) => {
...
const handleSubmit = () => {
  formRef.value?.validate((valid: boolean) => {
    ...
    MatrixApi.save(form.value, { success: true }).then(() => {
      handleRefresh(false, true)
...
const handleDelete = () => {
  TableUtil.selection(selection.value).then((ids: any) => {
    loading.value = true
    MatrixApi.delete(ids, { success: true }).then(() => {
      handleRefresh(false, true)
    }).catch(() => {
      loading.value = false
    })
...
const handleModel = (scope: any) => {
  router.push({ path: '/bi/design/matrix', query: { id: scope.row.id } }).catch(() => {})
}
</script>
...
<template>
  ...
  <el-card :bordered="false" shadow="never" class="fs-table-card">
    <div class="fs-table-toolbar flex-between">
      <el-space>
        <button-add v-permit="'bi:matrix:add'" @click="handleAdd" />
        <button-delete v-permit="'bi:matrix:delete'" :disabled="selection.length === 0" @click="handleDelete" />
      </el-space>
      <el-space>
    ...
    >
      ...
      <el-table-column label="操作">
        <template #default="scope">
          <el-button link @click="handleShow(scope)" v-permit="'bi:matrix:'">查看</el-button>
          <el-button link @click="handleEdit(scope)" v-permit="'bi:matrix:modify'">编辑</el-button>
          <el-button link @click="handleModel(scope)" v-permit="'bi:matrix:'">模型</el-button>
        </template>
      </el-table-column>
    </el-table>
