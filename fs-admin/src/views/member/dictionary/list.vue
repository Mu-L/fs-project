...
const columns = ref([
  ...
  { prop: 'leaf', label: '叶子' },
  { prop: 'ancestorId', label: '祖级', slot: 'ancestorId' },
...
const handleRefresh = (filter2query: boolean, keepPage: boolean) => {
  ...
  loading.value = true
  DictionaryApi.list(filters.value).then((result: any) => {
...
const handleSublevel = (scope: any) => {
  ...
  if (scope.row.ancestorId) {
    params.ancestorId = scope.row.ancestorId
    params.parentId = scope.row.id
  } else {
    params.ancestorId = scope.row.id
  }
...
<template>
  ...
  <el-card :bordered="false" shadow="never" class="fs-table-card">
    <div class="fs-table-toolbar flex-between">
      <el-space>
        <button-add v-permit="'member:dictionary:add'" @click="handleAdd" />
        <button-delete v-permit="'member:dictionary:delete'" :disabled="selection.length === 0" @click="handleDelete" />
      </el-space>
      <el-space>
