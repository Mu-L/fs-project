...
const handleRefresh = (filter2query: boolean, keepPage: boolean) => {
  ...
  loading.value = true
  MenuApi.list(filters.value).then((result: any) => {
  ...
  <el-card :bordered="false" shadow="never" class="fs-table-search" v-show="searchable">
    <form-search ref="filterRef" :model="filters">
      ...
      <template v-if="filters.advanced">
        ...
        <form-search-item label="链接" prop="url">
          <el-input v-model="filters.url" clearable />
        </form-search-item>
        <form-search-item label="打开方式" prop="target">
          <el-autocomplete v-model="filters.target" clearable :fetch-suggestions="query => UIUtil.filterSuggestions(MenuApi.target(), query)">
            <template #default="{ item }">{{ item.value }} - {{ item.description }}</template>
          </el-autocomplete>
        </form-search-item>
        <form-search-item label="图标" prop="icon">
          <el-input v-model="filters.icon" clearable />
        </form-search-item>
      </template>
    </form-search>
  </el-card>
