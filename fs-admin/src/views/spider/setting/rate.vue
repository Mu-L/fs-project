...
const columns = ref([
  { prop: 'id', label: 'ID' },
  { prop: 'name', label: '名称' },
  { prop: 'parallelByKey', label: '并行度' },
  { prop: 'concurrent', label: '并发数' },
  { prop: 'intervalMillisecond', label: '并发间隔' },
  { prop: 'perJob', label: '每作业' },
  { prop: 'perDomain', label: '每域名' },
  { prop: 'perNode', label: '每节点' },
  { prop: 'perProxy', label: '每代理' },
  { prop: 'haltToken', label: '延迟令牌' },
  { prop: 'haltTask', label: '延迟任务' },
...
const handleReload = () => {
  ...
  RateApi.info({ id: form.value.id }).then(result => {
...
const handlePublish = (scope: any) => {
  RateApi.publish(scope.row, { success: true }).then(result => {
    handleRefresh(false, true)
  }).catch(() => {})
}
const handleSubmit = () => {
  formRef.value?.validate((valid: boolean) => {
    ...
    RateApi.save(form.value, { success: true }).then(result => {
      handleRefresh(false, true)
...
const handleClear = (scope: any) => {
  ElementUtil.confirm(`确定清理'${scope.row.name}'配置？`).then(() => {
    RateApi.clear({ id: scope.row.id }, { success: true }).then(() => {
      handleRefresh(false, true)
    }).catch(() => {})
  }).catch(() => {})
}
...
<template>
  ...
  <el-card :bordered="false" shadow="never" class="fs-table-card">
    <div class="fs-table-toolbar flex-between">
      <el-space>
        <button-add v-permit="'spider:rate:add'" @click="handleAdd" />
        <button-delete v-permit="'spider:rate:delete'" :disabled="selection.length === 0" @click="handleDelete" />
      </el-space>
      <el-space>
    ...
    >
      ...
      <el-table-column label="操作">
        <template #default="scope">
          ...
          <el-button link @click="handleEdit(scope)" v-permit="'spider:rate:modify'">配置</el-button>
        </template>
      </el-table-column>
    </el-table>
  ...
  <el-drawer v-model="formVisible" :close-on-click-modal="false" :show-close="false" :destroy-on-close="true">
    <template #header="{ close, titleId, titleClass }">
      <h4 :id="titleId" :class="titleClass">请求频率</h4>
      <el-space>
    ...
    <el-form ref="formRef" :model="form" :rules="rules" label-width="auto">
      ...
      <el-form-item label="并行度">
        <el-input-number v-model="form.parallelByKey" />
      </el-form-item>
      <el-form-item label="并发数">
        <el-input-number v-model="form.concurrent" />
      </el-form-item>
      <el-form-item label="并发间隔">
        <el-space>
          <el-input-number v-model="form.intervalMillisecond" />
          <span>毫秒</span>
        </el-space>
      </el-form-item>
      <el-form-item label="延迟范围">
        <el-space direction="vertical">
          <el-checkbox v-model="form.perJob">每作业</el-checkbox>
          <el-checkbox v-model="form.perDomain">每域名</el-checkbox>
          <el-checkbox v-model="form.perNode">每节点</el-checkbox>
          <el-checkbox v-model="form.perProxy">每代理</el-checkbox>
        </el-space>
      </el-form-item>
      <el-form-item label="延迟策略">
        <el-space direction="vertical">
          <el-checkbox v-model="form.haltToken">延迟Token令牌</el-checkbox>
          <el-checkbox v-model="form.haltTask">延迟Task任务</el-checkbox>
        </el-space>
      </el-form-item>
